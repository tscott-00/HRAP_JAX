...
# Intermediate specs used as part of actual engine spec
...
# Engine computational structure, fixed attributes, and initial conditions
eng_spec = SelfPressurizedHybrid(
    tnk = SatTank(
        get_sat_props = fluid.bake_sat_coolprop('NitrousOxide', np.linspace(183.0, 309.0, 20)),
        V = (np.pi/4 * 4.75**2 * _in**2) * (7.0 * _ft),
        T0 = 294,
        m0 = 12.6, # TODO: init limit
        inj = Injector(
            CdA = 0.22 * (np.pi/4 * 0.5**2 * _in**2),
        )
    ),
    cmbr = Chamber(
        comb_props = ,
...
# Preprocess for compiled integration
z_meta = tracept.bake_tree(eng_spec)
integrator = tracept.odes.make_integrator(z_meta, tracept.odes.step_fe)

# Allocate initial engine state then set any dependent fixed attributes
eng0 = tracept.full(z_meta).prep()
...
t, eng = integrator(eng0, dt=1E-3, T=T)
t2  = time.time()
# t, x, xstack = fire_engine(s, x, dt=1E-3, T=T)
# jax.block_until_ready(xstack)
t, eng = integrator(eng0, dt=1E-3, T=T)
t3 = time.time()
print('done, first run (and compilation) took {a:.2f}s, second run took {b:.2f}s'.format(a=t2-t1, b=t3-t2))
...
# Plot thrust
axs[0].plot(t, eng.noz.thrust, label='sim')
