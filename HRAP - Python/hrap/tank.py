...
def liq_blowdown(tnk, fld, dP_dT, drho_v__dT, drho_l__dT):
    ...
    A = (tnk.m_liq*drho_l__dT/(rho_l**2) + tnk.m_vap*drho_v__dT/(fld.rho_vap**2)) / (1/fld.rho_vap-1/fld.rho_liq)
    B = -tnk.mdot/(fld.rho_liq/fld.rho_vap-1)
    C = -fld.Hv / ((tnk.m_liq+tnk.m_vap)*fld.Cp)
    Tdot = B*C / (1-A*C)
    mdot_evap = Tdot*A + B
...
def sat_vap_blowdown(tnk, fld, dP_dT, get_sat_props):
    delta = 1E-7 # FD step
    ...
    res = jax.lax.while_loop(lambda val: jnp.abs(val[0]) > 1E-9, Z_body, (1.0, fld.Z, fld.Z, tnk.T, tnk.T, m_2__m_1))
    T_2 = res[3]
    ...
    def __call__(self, tnk, fld, cmbr):
        ...
        def inj_vap_model():
            if inj.vap_model == 'Incompressible':
                ...
                Mcc = jnp.sqrt(fld.Z*1.31*188.91*T*(cmbr.P/tnk.P)**(0.31/1.31))
...
class Vent:
    ...
    def __call__(self, tnk, fld, env):
        ...
        if vnt.S == 0:
            ...
            Matm = jnp.sqrt(fld.Z*1.31*188.91*T*(env.Pa/tnk.P)**(0.31/1.31))
            Matm = jnp.minimum(Matm, 1)
            vnt.mdot = (vnt.CdA*tnk.P/jnp.sqrt(tnk.T))*jnp.sqrt(1.31/(fld.Z*188.91))*Matm*(1+(0.31)/2*Matm**2)**(-2.31/0.62)
...
class SatTank:
    ...
    m_vap:    jax.Array = Dynamic() #: mass of vapor
    rho_liq:  jax.Array = Dynamic() #: density of liquid
    rho_vap:  jax.Array = Dynamic() #: density of vapor
    ...
    def __call__(self, cmbr, env):
        ...
        tnk.m_liq = jnp.maximum((tnk.V - (tnk.m/fld.rho_vap))/ ((1/fld.rho_liq)-(1/fld.rho_vap)), 0.0)
        ...
        tnk.Tdot, tnk.Pdot = cond(tnk.m <= 0.0, lambda: (0.0, 0.0),
            lambda: cond(tnk.m_liq > 0.0,
                lambda: liq_blowdown(tnk, fld, dP_dT, drho_v__dT, drho_l__dT),
